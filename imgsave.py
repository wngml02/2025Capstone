...
DENOISED_MAT = "denoised_real_imag_50_sqrt_r2.mat"

OUT_DIR = Path("dn_50_rd_2")
...
print("⋯ 디노이즈 real / imag 로드")
m2 = sio.loadmat(DENOISED_MAT, simplify_cells=True)
den_real = m2["den_real"].astype(np.float32)
den_imag = m2["den_imag"].astype(np.float32)
...
# ─── 시각화 함수 ───────────────────────────────────────────────────────────
def visualize_and_save(mag_orig, mag_noisy, mag_den, mask, out_dir=OUT_DIR, slice_idx=Z_SLICE):
    print("⋯ echo-wise 5-column 시각화 생성")
    n_echoes = mag_orig.shape[-1]
    vmin, vmax = np.percentile(mag_orig[mask], (1, 99))
    diff_vmax = np.percentile(np.abs(mag_noisy - mag_orig)[mask], 99)
    ...
    for echo in range(n_echoes):
        ...
        # 0) Noisy magnitude
        axes[0].imshow(mag_noisy[:, :, slice_idx, echo], cmap='gray', vmin=vmin, vmax=vmax)
        axes[0].set_title('Noisy Magnitude')
        axes[0].axis('off')

        # 1) Denoised magnitude
        axes[1].imshow(mag_den[:, :, slice_idx, echo], cmap='gray', vmin=vmin, vmax=vmax)
        ...
        axes[3].imshow(diff_map[:, :, slice_idx], cmap='bwr', vmin=-diff_vmax, vmax=diff_vmax)
        axes[3].set_title('Diff (Denoised − Original)')
        axes[3].axis('off')

        # 4) Diff (Noisy – Original)
        diff_noisy_map = mag_noisy[:, :, :, echo] - mag_orig[:, :, :, echo]
        axes[4].imshow(diff_noisy_map[:, :, slice_idx], cmap='bwr', vmin=-diff_vmax, vmax=diff_vmax)
...
# ─── 시각화 실행 ────────────────────────────────────────────────────────────
visualize_and_save(mag_orig, mag_noisy, mag_den, mask)
